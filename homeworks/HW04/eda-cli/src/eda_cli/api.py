...
class FlagsReportResponse(BaseModel):
    """Ответ в виде флагов"""
    flags: Flags
...
@app.post(
    ...
    response_model=FlagsReportResponse,
    tags=["quality"],
    summary="Принимает CSV-файл и возвращает полный набор флагов качества.",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> FlagsReportResponse:
    """
    Принимает CSV-файл (аналогично /quality-from-csv) и возвращает полный набор флагов качества.
    """
    ...
    flags_all = {"flags": {
        key:bool(value)
        for key, value in flags_all.items() if isinstance(value, bool)
        }
    }
    ...
    result = FlagsReportResponse(**flags_all)
