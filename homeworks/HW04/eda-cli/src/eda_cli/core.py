...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # 1. флаг, показывающий, есть ли колонки, где все значения одинаковые
    ...
    # 2. флаг, сигнализирующий, что есть категориальные признаки с очень большим числом уникальных значений
    has_high_cardinality_categoricals = False
    for column in summary.columns:
        if column.dtype in ("category", "object"):
            if column.unique > 100: # порог 100 уникальных значений
    ...
    if summary.n_cols > 100:
        score -= 0.1
    # оценка качества на основе добавленных эвристик
