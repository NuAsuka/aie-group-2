...
- **Название проекта:** предсказание количества товара/категории на следующий день/неделю для магазинов ритейла
- **Автор:** Тапилин Виктор Викторович
- **Группа:** ИНБО-21-24
- **Контакт:** @nuasuka

- **Краткое описание:** 

Проект посвщён предсказанию количества товара/категории на следующий день/неделю для магазинов ритейла. Сервис будет полезен менеджерам, логистам, маркетологам ритейловых компаний тем, что позволяет заранее узнать спрос на выбранные товары. Решение такой задачи позволит экономить деньги на логистике и складских помещениях за счёт оптимизации излишков и дефицита товара.

Используюется открытый датасет. В качестве моделей используется линейная регрессия, CatBoost и LSTM / GRU.

Сервис реализован через REST API для получения прогноза на выбранный период (день/неделя/месяц).
...
Используются синтетические данные с Kaggle (Retail Store Inventory Forecasting Dataset) о разничных магазинах. 
Основные данные лежат в `data/`:
- `retail_store_inventory.csv` - основной датасет на котором производится обучение и проверка моделей.
